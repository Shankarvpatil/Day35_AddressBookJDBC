...
public class AddressBookDBService {
    ...
    private List<Person> getPersonData(ResultSet resultSet) {
        ...
        try {
            while (resultSet.next()) {
                ...
                personList.add(new Person(id,firstName,lastName,address,city,state,zip,phoneNo,email,entryDate));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    private int updateAddressBookDataUsingStatement(String firstName, String phoneNo) {
        String sql = String.format("update address_book set phoneNo = '%s' where firstName = '%s';", phoneNo, firstName);
        try (Connection connection = this.getConnection()) {
            Statement statement = connection.createStatement();
